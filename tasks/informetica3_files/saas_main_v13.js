...
Recruit_CompanyPage = new function () {
	...
	$instance.LoadSwitch = function () {
        $.getScript(theme_url + "/javascript/bootstrap-switch.min.js")
                .done(function () {
...
Bosch_Commonfunction = new function () {
	...
	$instance.commonChosen = function () {
		...
		$.getStylesheet(theme_url + "/chosen_min.css")
...
$instance.init = function () {
	...
	$instance.ProgressBarRound = function () {
		$.getScript(theme_url + "/javascript/jQuery-plugin-progressbar.js")
                .done(function () {
...
$(document).ready(function() {
	...
	$('#home_page_season_url').keyup(season_home_url_check);
	  ...
	  $('#editor-box .expand-collapse-btn').click(function(){
        ...
        $(document).on('click','#editor-box .user-actions .keyboard-btn',function(){
			$("#keyboard-bindings").modal('show');
		});
		
        $(document).on('click','#editor-box .user-actions .settings-btn',function(){
			$(this).toggleClass("active");
		...
		$('#settings-dropdown .btn-group .btn').each(function(){
			...
			$(document).on('click','#downloadUserCode',function(){
            ...
            $(document).on('change','#change_editor_themes',function(){
            ...
            $(document).on('keyup','#tab_space',function(){
			...
			success: function(data){
				...
				if(msg.status == 'error') {
                    ...
                    if(page_number == 'none'){
                        ...
                        $('#cahllenge-test-submit').modal('show');

                    }else{
                        var action_url = contest_redirect_url+'/'+invitation_id+'/'+page_number;
                        window.location.href = action_url;
                    }
				} else {
					$("<span class='error_msg'> Could not upload file </span>").insertAfter(".btn-file");
		...
		success: function(data) {
			...
			if(status == 'return') {
			  ...
			  $.each(msg, function (index, value) {
				...
				$( "#login-form .alert-danger").show().html(msg.message); 
			} else {
				window.location.href = window.location.href;
			}		
		}
...
function checkSaasRegister() {
    ...
    if(txtCaptcha.length == 0){
        $("<span class='error_msg'> Please verify captcha!.</span>" ).insertAfter("#register_token_form .capcha-box");
	...
	$.ajax({
		...
		success: function(data) {
			...
			var status = msg.status;
			if(status == 'exist') {
			  $( "#signup-form .alert-danger").show().html(msg.message); 			  
			  ...
			  //$( "#signup-form .alert-danger").show().html(msg.message); 
				...
				$( "#register_token_form input[type=text], #register_token_form input[type=password]").val("");
				$( ".alert-verification").show().html(msg.message); 
				setTimeout(function() {$('#login-register-popup').modal('hide');}, 4000);
...
$(document).ready(function(){
    $(document).on('click','#save-ctf-information', function(){
    	var question_id = $('#question_id').val();
		var invitation_id = $('#invitation_id').val();
		var cft_value = $('#cft_flag_value').val();
		
		
		var answer_option ;
		var platform_type = $('#platform_type').val(); 
		var test_action_url = $('#test_action_url').val(); 
		var filename = $('#ctfFileupload').val();
		var check=0;
		if($('#attempted_option').is(':visible')){
    	...
    	if(cft_value=='' || cft_value=='undefined'){
    		$('#ctf-body-content p').text("Please enter the ctf Flag before submitting");
    		$('#submit-incomplete-ctf').modal('show');
    		check =1;
    	}
    	/*else if(answer_option == '' || answer_option == 'undefined'){
    		$('#ctf-body-content p').text("Please enter the writer text before submitting");
    		$('#submit-incomplete-ctf').modal('show');
    	}*/
		...
		var postString ;
		if(answer_option != ''){
			postString = 'question_id='+question_id+'&invitation_id='+invitation_id+'&answer_options='+answer_option+'&cft_value='+cft_value+'&filename='+filename;
		}
		else {
			postString = 'question_id='+question_id+'&invitation_id='+invitation_id+'&cft_value='+cft_value+'&filename='+filename;
		}
		...
		$.ajax({
			type: "POST",
			url: base_url+'/ajax_files/saas_candidate_function.php?action=websecurity_save_solution', 
			data: postString,
			success: function(data){
				//data = $.trim(data);
				var msg = data; 
				...
				if(msg.status == 'error') {
					...
					$('#save-ctf-information').text("Saved");
					// if(ajax_list == 1) {
                        
     //                    var modules = 'SaasCandidateQuestionDetail';
     //                    var next_question_token = $('#next_question_token').val();
     //                    var page_number = $('#next_page_count').val();
     //                    var params = {
     //                         question_no : page_number,
     //                         invitation_id : next_question_token
     //                    };
     //                    if(page_number) {
...
$(document).ready(function(){
    $(document).on('click','#check-ctf-information', function(){
    	...
    	if(cft_value=='' || cft_value=='undefined'){
    		$('#ctf-body-content p').text("Please enter the Flag before Verifying");
    		$('#submit-incomplete-ctf').modal('show');
    		check =1;
    	}   		
			
		var	postString = 'question_id='+question_id+'&invitation_id='+invitation_id+'&cft_value='+cft_value;	
        var objButton = $(this); 
        
		if(check!='1'){
			$("#check-ctf-information").addClass( "disabled" );
			try	{
				$.ajax({
					type: "POST",
					url: base_url+'/ajax_files/saas_candidate_function.php?action=ctf_flag_check', 
					data: postString,
					success: function(data){
						var msg = data;             	                   
						...
						if(msg.status == 'error') {
							...
							$('.submit-error .modal-body p').html(msg.error);
							$('.submit-error').modal('show');
		                    $("#check-ctf-information").removeClass("disabled");
						...
						} else if(msg.status == 'success' && msg.flag_check == 'pass') {

							$('#ctf-body-content p').text("Entered flag is correct");
		    				$('#submit-incomplete-ctf .modal-title').hide();
		    				$("#check-ctf-information").attr('disabled', 'disabled');
		    				$("#check-ctf-information").addClass('disabled');
		    				$('#submit-incomplete-ctf').modal('show');

						} else if(msg.status == 'success' && msg.flag_check == 'fail') {

							$('#ctf-body-content p').text("Entered flag is not correct"); //and you have "+msg.count+ " attempts left
		    				$('#submit-incomplete-ctf').modal('show');		
		                    $("#check-ctf-information").removeClass("disabled");

						}else if(msg.status == 'success' && msg.flag_check == 'error') {

							$('#ctf-body-content p').text("Your flag check limit is exceeded.");
		    				$('#submit-incomplete-ctf').modal('show');
		                    $("#check-ctf-information").removeClass("disabled");
	...
	try	{
		$.ajax({
			...
			success: function(data){
				var msg = data; 
                $(".save-sentence-question").removeClass( "disabled" );
                $(".save-sentence-question").text("Saved");
...
$(document).ready(function(){
	...
	var test_action_url = $('#test_action_url').val(); 
	var practice = $('#practice').val(); 
	if($("#question_category").is(':visible')){
...
$(document).on('click','#submit_button_subjective', function(){
	...
	var platform_type = $('#platform_type').val(); 
	var page_number = $('#next_page_count').val(); 
	var invitation_id = $('#current_encrypt_token').val();
	...
	try{
            ...
            $.post(url,{'answer_option':answer_option,'invitation_id':invitation_id},function(data){ 
                var msg =data; 
                $("#submit_button_subjective").removeClass("disabled" );
                //$("#submit_button_subjective").text("Saved");
                                                
                if(msg.status == 'error') {
                    $("#submit_button_subjective").removeClass( "disabled" );
                    //$("#submit_button_subjective").text("Save Answer");
                    $('#submit-error .modal-body p').html(msg.error);
                    $('#submit-error').modal('show');
                } else if(msg.status == 'timeout') {
                    alert('Contest Time is over. Click OK to see the result');
...
$(document).ready(function(){
		...
		$.ajax({
			...
			success: function(data){
				...
				if(msg.status == 'error') {
					...
					$("#save-front-question").val("Submit");
					$('.submit-error .modal-body p').html(msg.error);
					$('.submit-error').modal('show');
	...
	$(document).on('click','#render-front-question', function(){
	    var question_id = $(this).data('question_id');
		...
		$.ajax({
			...
			success: function(data){
				...
				if(msg.status == 'error') {
                    ...
                    $('#check_user_submission').val('Y');
                    var iframePath = $("#iframePath").val();
                    $("#Frame").attr('src',iframePath);
                    var iframe = document.getElementById('Frame');
					iframe.src = iframe.src;
					$('#front-end-render').show();
					setTimeout(function(){ 
                    	var href1= "#front-end-render";
                        var target=$(href1).parents(".mCustomScrollbar"); 
                        target.mCustomScrollbar("scrollTo",href1);
                        $('html, body').animate({scrollTop: $("#front-end-render").offset().top - 25}, 1000);	
                    }, 800);
...
function SetDivPosition(){ 
	...
	function updateUserSubjectiveCode() {
		...
		var attempt_encrypt_token = $('#current_encrypt_token').val();
...
function updateUserCtfData() {
 
		try {
		if($("#text_content").is(':visible')) {
			var nicInstance = nicEditors.findEditor('text_content');
        	var template_content=nicInstance.getContent(); 
        	var writeUp = template_content.replace(/&nbsp;/g, ' ');	
		}
        
        var ctf_code = $("#cft_flag_value").val();
        var filename = $("#file-upload-csv").text();
		if(!ctf_code && !filename && !writeUp) {
			return false;
		}

		var attempt_encrypt_token = $('#next_encrypt_token').val();
		
		var url = base_url+'/ajax_files/saas_candidate_function.php?action=save_ctf_code_data';
		
		$.post(url,{'encrypt_token':attempt_encrypt_token,'ctf_code':ctf_code, 'writeUp':writeUp,'filename':filename}, function(response) {
			var msg = response; 
				if(msg.status == 'success') {
					//saved
				}
...
function changeClientPassword(admin_id) {
  ...
  $("#change_client_password").modal('show');
        ...
        editor.onPaste = function() {  
            var key_ctrl_disable = 'Copy/Paste functionality has been disabled';
            $('#disable_ctrl_key_combination .modal-body p').html(key_ctrl_disable);
            $('#disable_ctrl_key_combination').modal('show');
            return "";
        }
	...
	function restrictUserRightClick(){
        ...
        function clickIE() {
            if (document.all) {
                $('#disable_ctrl_key_combination .modal-body').html('<p>Right click disabled</p>');
                $('#disable_ctrl_key_combination').modal('show');
            }
        }
        ...
        var div_mcq_section = document.getElementById("mcq-section-area")
        disableSelection(div_mcq_section);
...
function compile_test(action, contest_id, encrypt_token, testcase_type, platform_type, want_to_confirm) {
	...
	/*if(write_code == '' && want_to_confirm != 'N') {
	...
	if(action == 'submit') {
            ...
            if(form_post == 2){ // for virtualcampus/tg3
                $("#after-submit-clck").modal({
                    backdrop: 'static',
                    keyboard: false
                });
                    
                $('#after-submit-clck').on('hide.bs.modal', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
                
                $('#after-submit-clck .modal-body').html('<p class="text-center tg-loader"><img src="' + THEME_PATH + '/images/TG-Loader.gif"></p>');
                $('.submit-test-response-data').val("Submitting...");
                $('.submit-test-response-data').addClass('disabled');
...
function event_execution() {
    
     window.clearTimeout(UE);
     window.clearTimeout(SD);
                                    
    var encrypt_token = $('#new_encrypt_token_data').val();
    
    $('#submit-test #submit_test_response').text("Submitting...");
    $('#submit-test #submit_test_response').addClass('disabled');
    ...
    var encrypt_attempt_id =  $('#encrypt_attempt_id').val();
    var event_name = $("#event_name").val();
    var company_page_url = $('#company_page_url').val();
    $('#proctoring_message_prompt').modal('hide');
    if(userAttemptStatus != 'C'){
    
    $.ajax({
          type: "POST",
          url: base_url+'/codejudge_coding_question_result.php?attempt_id='+encrypt_attempt_id, 
          data: {'encrypt_token':encrypt_token, event_name:event_name, company_page_url:company_page_url},
          cache: false,
        success: function(data){
            if($('#webcam_container').length){
               // $.scriptcam.closeCamera();
            }
            $('#submit_test_response').text('Submitting');
            setTimeout(function(){ 
                console.log('success');

                var attempt_response = $.trim(data);
                if(attempt_response == "C"){
	                $.ajax({
				          type: "POST",
				          url: base_url+'/ajax_files/saas_candidate_function.php?action=show_marks_details', 
				          data: {'encrypt_token':encrypt_token, event_name:event_name},
				          cache: false,
				          dataType: 'html',
				          success: function(data){
				              setTimeout(function(){ 
    ...
    $.ajax({
          ...
          success: function(data){
              setTimeout(function(){ 
                $('.bootbox').modal('hide'); 
                 $('#after-submit').find('.modal-body').html('');
              //  $("#after-submit-clck [data-dismiss=modal]").trigger("click");
                $('#after-submit-clck').hide();
               // $('.modal-backdrop').hide();
                 $('#after-submit-clck').modal('hide'); 
                ...
                $('#after-submit').find('.modal-body').html(data); 
            }, 500);
...
function compile_test_execution(action,contest_id,encrypt_token,testcase_type,platform_type,language) {
					...
					try{
						...
						if(action == 'compile') {
							...
							$('#submit_code .btn').addClass("disabled");
							$('#is_compile').val(0);
						}
                      ...
                      //show modal watch for submit case only   2 minutes 
                      if(action == 'submit') {
                            SubmitWaitClock();
                      }
                      ...
                      $.ajax({
                              type: "POST",
                              url: url, 
                              data: {'code':code,'action':action,'contest_id':contest_id, 'language':language, 'encrypt_token':encrypt_token, 'testcase_type':testcase_type,'own_testcase_input':own_testcase_input,'platform_type':platform_type,'isBotChallenge':isBotChallenge,'candidate_encrypt_token':candidate_encrypt_token},
                              cache: false,
                              timeout: 100000,
                              success: function(data){
                                  
                                $('#submit_code .btn').val("Submit Code");
                                
                                //hide modal watch 2 minutes for submit only   
                                if(action == 'submit') {
                                    HideWaitClock();
                                }
                                ...
                                $('.btn').removeClass("disabled");
                                                        ...
                                                        //For vcl/tg3 changes
                                                        if(form_post == 2 && action == 'submit'){
                                                           
                                                            var res_data = $.trim(data)
                                                            if(res_data == ''){
                                                                compile_test_execution(action,contest_id,encrypt_token,testcase_type,platform_type,language);
                                                            }
                                                            
                                                            var resdata = res_data.split('*||*'); 
                                                            if(resdata[1] == 'failure' || (resdata[1] == 'timeout' && userAttemptStatus!='C')) {
                                                                compile_test_execution(action,contest_id,encrypt_token,testcase_type,platform_type,language);
                                                            }
                                                            if($('#webcam_container').length){
                                                                $('#proctoring_message_prompt').modal('hide');
                                                            ...
                                                            if($("#full-screen-question").hasClass("normal-view")){
                                                                $("#editor-box #own_testcase .form-control").css("min-height", $(window).height() - ($("#coding-platform-head").innerHeight() + $("#editor-box header").innerHeight() + $("#editor-box .editor-footer").innerHeight() + 20));
                                                            }else{
                                                                $("#editor-box #own_testcase .form-control").css("min-height", $(window).height() - ($("#coding-platform-head").innerHeight() + $("#editor-box header").innerHeight() + 130));
                                                            }
							...
							if($.trim(data) == '') {
								...
								var res = new_data.split('*||*');
									...
									if(action == 'compile') {
										...
										$('#code_result').addClass('success-msg');
										//$('#code_result').html('Code Submitted successfully.').show();
                                                                                $('#code_result').html('Some technical error has occured. Please try again.').show();
										$('#submit_code .btn').val("Submit Code");
									}
									...
									$( "#code_result span" ).before( UserCompileoutputLog );
									   ...
									   if($('#webcam_container').length){
										...
										if(form_post == 1) {
											var initialCodeLen = $('#defaultlen').val();
											var newCodeLen = code.length;
											
											if(initialCodeLen != newCodeLen){
												$('#check_user_submission').val('Y');
												$('.submit-complete').removeAttr('data-toggle');
												$('.submit-complete').removeAttr('data-target');
											}
                                            ...
                                            if(page_number != 'none'){
                                                window.location.href = action_url +'/'+page_number;	
                                            }else{
                                                $('#check_user_submission').val('Y');
                                                $('.submit-complete').removeAttr('data-toggle');
												$('.submit-complete').removeAttr('data-target');
                              ...
                              error : function() {
                                  //set auto submit loader closed 
                                  //submit case only 
                                  //Set attempt status to Completed in case of time exceed
                                  	
                                  HideWaitClock();
                                  if(form_post == 1) {

                                  	//update attempt status to complete
                                  	update_attempt(encrypt_token);
	...
	function update_attempt(encrypt_token){
		$.ajax({
			type: "POST",
			url: base_url+'/ajax_files/saas_candidate_function.php?action=update_attempt',
	...
	function screen_capture() {
		 ...
		 if(question_category == 'coding') {
			var capture_class = '.inner-coding-area';
		 } else {
			var capture_class = '.inner-coding-area';
		 }
			...
			onrendered: function (canvas) {
				...
				var encrypt_token = $("#encrypt_token").val();
					$.ajax({
						...
						data: 'img_val='+encodeURIComponent(cap_img_val)+'&encrypt_token='+encrypt_token,
						success: function(data){
							/*data = $.trim(data);
							var msg = $.parseJSON(data); 
							if(msg.status == 'save') {
	...
	function userLooseFocus(encrypt_token,question_id,platform_type,out_movement_allowed_number, category_key){
        ...
        if ( document.hasFocus() ) {
            ...
            bootbox.alert({ 
				...
				title: "Warning",
				backdrop: true,
				message: "<p>You are not allowed to move out of current window during this contest!</p>", 
				callback: function(){	
                    console.log('dd=>'+window_warning);                
                    window_warning = window_warning+1;
                    var today = new Date();
                    var expiry = new Date(today.getTime() + 1 * 24 * 60 * 60 * 1000);
                    var name = 'window_warning';
                    SetCookie(name, window_warning, expiry);
                    
                    ANT = setInterval( function() { userLooseFocus(encrypt_token,question_id,platform_type,out_movement_allowed_number, category_key); }, 200 ); 
            ...
            if(window_warning >= out_movement_allowed_number){
                
                $('#disable_combination .modal-body p').html('<p>We are auto submitting your contest as you have not followed instructions properly as you have moved out of the contest window.</p>');
                $('#disable_combination .modal-title').hide();
                ...
                setTimeout(function() {
                    ...
                    if(category_key == 'coding' || category_key == 'approximate_solution' || category_key == 'botchallenge'){
                        $('#form_post').val('3');
                        compile_test_execution('submit',question_id,encrypt_token,'tg_testcase',platform_type,language);
                    }else{
                        //
                    }
                    $('#disable_combination .modal-content').html('<div class="tg-loader text-center" style="padding:20px;"> <p> Please wait processing... </p> <p> <img src="' + theme_url + '/images/TG-Loader.gif"> </p> </div>');
                    $('#codejudge_requirement').submit();
                }, 1000); 
            } 
...
function SubmitWaitClock() {
    window.StartTime = 100;  //do not remove global varibale
    setWaitCountDown(); // run function once at first to avoid delay
    window.SWD = setInterval(setWaitCountDown, 1000);
    $('#TechGigbootStrapModal .modal-body').html('<div class="text-center"> <p> Your code evaluation is in progress.  <br /> Please wait...  <br />  <br /> <span class="code-wait-time">  </span>  </p> </div>'); 
    $('#TechGigbootStrapModal .modal-dialog').addClass('modal-sm');  
    $("#TechGigbootStrapModal").modal({
        backdrop: 'static',
        keyboard: false
    });
...
function setWaitCountDown () {
    window.StartTime--;
    $(".code-wait-time").html(window.StartTime+ '<br /> Seconds');
    if (window.StartTime == 0) { 
        clearInterval(window.SWD);
        HideWaitClock();        
    } 
}
